...
// ObservedRelease is an immutable copy of a release.Release as observed to be
// written to a storage driver.Driver by an Observator.
...
// into a new immutable ObservedRelease while omitting all chart data except
// metadata.
...
// persisted release.Release objects as immutable ObservedRelease by their
// Helm storage key.
//
// This allows for observations on persisted state as performed by you,
...
type Observer struct {
	...
	// to persist to, and retrieve data from.
	driver driver.Driver
	// releases contains a map of immutable ObservedRelease objects indexed by
	// makeKeyFunc key.
...
func (o *Observer) LastObservation(name string) (ObservedRelease, error) {
	...
	return o.releases[o.makeKeyFunc(name, candidates[len(candidates)-1])], nil
}
...
func (o *Observer) ObserveLastRelease(name string) (ObservedRelease, error) {
	...
	return o.releases[key], nil
}

// wlock locks Observer for writing.
func (o *Observer) wlock() func() {
	o.mu.Lock()
	return func() { o.mu.Unlock() }
}

// rlock locks Observer for reading.
