...
func BuildClientGetter(config *rest.Config, namespace string, opts ...ClientGetterOption) genericclioptions.RESTClientGetter {
	...
	if ia := o.impersonateAccount; ia != "" {
		config := *config
		SetImpersonationConfig(&config, namespace, ia)
		return NewInClusterRESTClientGetter(&config, namespace)
	}
	return NewInClusterRESTClientGetter(o.config, namespace)
}
